...
В карточке подключения нажмите **Ссылка и QR**: панель соберёт профиль в официальном формате OpenFlux `openflux://v1/`. Android v1.1.0 может открыть ссылку или считать QR-код. Если в профиле есть Direct, укажите публичный IP-адрес или домен VPS. Для Cups.online сначала запустите подключение, чтобы появился код комнат. Ссылка и QR содержат общий ключ шифрования — передавайте их только своим устройствам. Панель может экспортировать автоматически созданный ею ключ; если вы указали внешний файл ключа, он не раскрывается через эту функцию.
