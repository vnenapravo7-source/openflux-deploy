...
В форме подключения можно включить **Защищённую сессию** и указать до восьми транспортов с приоритетами. Панель передаст ядру `--transports`, адреса документов, `--negotiate`, `--max-packet-size` и отдельное хранилище cookies для подключения. Приоритетный транспорт используется первым; при потере связи ядро переключается на другой. Для Direct TCP оставьте поле порта пустым: панель назначит свободный порт из диапазона 39000–39015 и не даст двум подключениям занять один порт.
