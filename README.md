...
Для Yandex Board выберите одноимённый транспорт и вставьте гостевую ссылку вида `https://boards.yandex.ru/guest/?hash=...`. Серверный OpenFlux должен быть обновлён до версии, содержащей транспорт `boards`; в мобильном клиенте тоже должна быть поддержка этого транспорта — добавление пункта в панели не обновляет приложение на телефоне.

### Режим OpenFlux 0.0.5

В форме подключения можно включить **Защищённую сессию** и указать до восьми транспортов с приоритетами. Панель передаст ядру `--transports`, адреса документов, `--negotiate`, `--max-packet-size` и отдельное хранилище cookies для подключения. Приоритетный транспорт используется первым; при потере связи ядро переключается на другой. Для Direct TCP задайте адрес прослушивания `:39000`–`:39015`; Docker публикует эти TCP-порты, но firewall VPS тоже должен их пропускать. Два подключения не могут занимать один Direct-порт.

Новый режим требует Batched и одинаковый секретный ключ шифрования у сервера и клиента. Путь к файлу ключа указывается в форме. Клиент должен поддерживать `--negotiate` и иметь тот же набор транспортов, адреса и ключ. В многотранспортном режиме клиенту не следует задавать старый общий `--url`: он меняет контекст рукопожатия. Существующие подключения остаются в прежнем режиме и не меняют протокол автоматически. UDP на стороне ядра доступен, но на iOS включается отдельно в мобильном приложении. Автоматическое решение капчи и обмен cookies требуют поддержки IPC в приложении; веб-панель не заменяет этот мобильный компонент.
