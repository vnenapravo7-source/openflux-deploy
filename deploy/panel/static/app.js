...
function openConnection(c){
  ...
  $("#sessionMode").checked=!!c?.transports?.length;$("#sessionTransportRows").replaceChildren();(c?.transports?.length?c.transports:[{type:c?.transport||"yandex",url:c?.url||"",priority:100}]).forEach(addSessionRow);$("#directListen").value=c?.direct_listen||"";$("#legacyDirectListen").value=c?.direct_listen||"";$("#maxPacketSize").value=c?.max_packet_size||"";
