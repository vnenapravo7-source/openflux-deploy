...
if [ ! -x "$STATE_DIR/bin/openflux" ] || { [ -f "$STATE_DIR/server-seed-next-start" ] && ! cmp -s /usr/local/lib/openflux-deploy/upstream-version.seed "$STATE_DIR/upstream-version"; }; then
...
if [ ! -x "$STATE_DIR/bin/openflux-panel" ] || { [ -f "$STATE_DIR/panel-seed-next-start" ] && ! cmp -s /usr/local/lib/openflux-deploy/panel-revision.seed "$STATE_DIR/panel-revision"; }; then
